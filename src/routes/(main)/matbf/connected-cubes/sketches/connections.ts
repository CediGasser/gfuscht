...
export function sketch(p: p5) {
  ...
  p.draw = () => {
    ...
    drawConnections(sample1, 2)

    p.stroke('indigo')
    sample2.forEach((p) => hexagonBestagon(p.x, p.y, radius - margin))
    drawConnections(sample2, 1)

    p.stroke('green')
    sample3.forEach((p) => hexagonBestagon(p.x, p.y, radius - margin))
    drawConnections(sample3, 0)

    p.noLoop()
  }

  function drawConnections(hexagons: p5.Vector[], direction: 0 | 1 | 2) {
    let angles = [p.PI / 6, (p.PI / 6) * 5, p.PI / 2]
    let offsetAngles = [p.PI / 6, (p.PI / 6) * 5, p.PI * 1.5]

    const offsetAngle = offsetAngles[(direction + 2) % 3]
    const offsetVector = p5.Vector.fromAngle(offsetAngle).mult(radius / 2 - 2)
    ...
    for (let i = 0; i < count; i++) {
      for (let j = 0; j < count; j++) {
        ...
        const sub = p5.Vector.sub(one, other)

        const vectorAngle = p.createVector(1, 0).angleBetween(sub)
        const threshold = 0.01

        if (
          (Math.abs(vectorAngle - angles[direction % 3]) < threshold ||
            Math.abs(vectorAngle - angles[(direction + 1) % 3]) < threshold) &&
          p.random() < lineConnectChance &&
          sub.mag() < maxConnectionLength
        ) {
          p.strokeWeight(4)
          p.point(oneOffset)
          p.point(otherOffset)
